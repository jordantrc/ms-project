...
class DataPackager:
	...
	def __init__(self, feature_model=None, flip=False):
		# feature model
		...
		# subscribers
		QUEUE_SIZE = 1
		self.sub_img = rospy.Subscriber(topic_names[0], 
			Image, self.imgCallback,  queue_size = QUEUE_SIZE)
		self.sub_aud = rospy.Subscriber(topic_names[1], 
	...
	def formatOutput(self, name=""):
		...
		if(len(self.__imgStack) > 0):
			img_stack = self.formatImgBatch(self.__imgStack, name)
